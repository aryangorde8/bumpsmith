...
| [#48](https://github.com/aryangorde8/bumpsmith/pull/48) | reviewed | 0 | 2 |
| [#49](https://github.com/aryangorde8/bumpsmith/pull/49) | reviewed | 0 | 2 |
...
## 187 · A disposition that renders as an empty cell

Found by reading the log on github.com rather than in an editor, which is the
only place the defect exists. GitHub ends a table cell at every `|` that is not
backslash-escaped, and an inline code span does not protect it. Two rows carried
shell that contains the character:

- 178 — `cat MARKER 2>/dev/null \|\| echo __ABSENT__`, two pipes
- 143 — `--jq '… \| length'` and `jq -s 'add \| map(…) \| length'`, three

A row offering more cells than its header declares does not wrap or overflow.
The surplus is dropped, and the surplus begins at the **Disposition** column. So
178 — a `High` — displayed as a finding with no disposition and no location, in
the table whose promise is that a finding closed without a visible disposition is
indistinguishable from one nobody read. That promise was false for two rows, in
the direction that flatters nobody.

The file was never wrong. Every disposition was written, committed and merged,
and `git show` prints all of it. What failed is the assumption that a table which
parses is a table that renders, and the checks over it inherited it: the index
was numbered, the provenance summed, the README's total matched, and none of
those can see a cell GitHub throws away. The new check is the cheap one that can
— count the pipes a row escapes against the width its separator declares — and it
covers the README's tables too, because nothing about the mistake is particular
to this file.

## 188 · "The last three" was never the last three

The README's review paragraph described the outside audit as the source that
*"raised the last three"* findings. It raised 182, 183 and 184. Finding 185 —
Qodo's second round on the fix for 184 — was added by the same pull request that
wrote the sentence, so it was false on arrival rather than later; 186 and 187
have since put two more rows behind it.

The cost is not that the number is wrong. It is that the sentence sends a reader
to the wrong rows: follow it today and you land on 186, an empty-review report,
and credit it to an audit that never saw the repository. Provenance is the one
thing the log's three-way split is for.

Finding 181 is the same defect, eight rows earlier, and its entry reads *"a
positional reference, written into the same commit that made it wrong"*. Its
lesson was applied to the paragraph beside the TrueForge table and not to this
one — which is this log's most-repeated shape in its most-repeated form, a fix
landed in one place and left standing in another. The sentence now names 182 to
